...
55. Expand the **Advanced Details** section, and for **User data - *optional***, paste the following script:
    ...
    💬 This script permits login by using a password. It is included to help make the lab steps shorter but is not recommended for normal instance deployments.
...
67. When prompted for a password, enter `lab-password`.
...
68. Run the following command:
    ...
    You should see results similar to the following. The architecture is configured correctly and the previous ping didn't work because of something on amazon's end.

    ```
    PING 8.8.8.8 (8.8.8.8) 56(84) bytes of data.
    64 bytes from 8.8.8.8: icmp_seq=1 ttl=115 time=8.00 ms
    64 bytes from 8.8.8.8: icmp_seq=2 ttl=115 time=7.65 ms
    64 bytes from 8.8.8.8: icmp_seq=3 ttl=115 time=7.66 ms
    ```
