...
- DB engine version: 10.5.13 (* 10.5.27)
...
- **Note:** About the engine version, any version after 11.4 requires another security step, so in order to avoid that, use any version older than 11.4.
...
```
mysql --user=root --password='Re:Start!9' \
--host=<RDS Instance Database Endpoint Address> --ssl \
< cafedb-backup.sql
```
